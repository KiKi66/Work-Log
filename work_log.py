...
def date_search():
  ...
  with open('work_log.csv') as csvfile:
    ...
    if date_option == '1':
      ...
      for row in reader:
        if ((datetime.datetime.strptime(row['date'], '%m/%d/%Y').timestamp() >= date1[0].timestamp()) and (datetime.datetime.strptime(row['date'], '%m/%d/%Y').timestamp() <= date2[0].timestamp())):
          search_result.append(row)
...
def display_result(results):
  ...
  for result in results:
    print('''Result {} of {}\n
    Task name: {}, Spent time: {}, Date: {}, Notes: {}\n'''.format(count, len(results), result['name'], result['time'], result['date'], result['notes']))
    selection = None
...
def edit(result_dict):
  ...
  while not new_infor or new_infor.isspace():
    ...
    if field == '3':
      try:
        datetime.datetime.strptime(new_info, '%d/%m/%Y')
      except ValueError:
